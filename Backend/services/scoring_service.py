...
def calculate_results(attempt):
    ...
    total_marks   = sum(
        a.question.marks for a in answers.select_related('question')
    )
    score_earned  = sum(a.marks_awarded for a in answers)
    percentage    = round(
        (score_earned / total_marks * 100) if total_marks > 0 else 0,
        2
    )
    is_passed     = percentage >= attempt.quiz.pass_percentage
    submitted_at  = timezone.now()
    is_late       = (
        attempt.expires_at and submitted_at > attempt.expires_at
    )
    time_taken    = int(
