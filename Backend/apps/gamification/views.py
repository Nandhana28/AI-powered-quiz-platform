...
class UserGameProfileView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            return error_response(message="Game profile not found", status_code=404)
...
class PersonalBestListView(APIView):
    ...
    def get(self, request):
        ...
        bests = (
            PersonalBest.objects.filter(user=request.user)
...
class TopicStatListView(APIView):
    ...
    def get(self, request):
        ...
        stats = (
            UserTopicStat.objects.filter(user=request.user)
            .select_related("topic")
...
class WeeklyChallengeView(APIView):
    ...
    def get(self, request):
        ...
        if not quiz:
            return success_response(data=None, message="No active weekly challenge")

        from apps.quizzes.serializers import QuizListSerializer

        return success_response(data=QuizListSerializer(quiz).data)
