...
class UserGameProfile(models.Model):
    ...
    level = models.CharField(max_length=15, choices=LEVEL_CHOICES, default="novice")
...
class UserTopicStat(models.Model):
    """
    Fix 6 — tracks stats per user + topic + difficulty so easy/medium/hard
    performance is never averaged together into a misleading overall score.
    The 'all' difficulty row is the aggregate across all difficulties.
    """
    ...
    DIFFICULTY_CHOICES = [
        ...
        ("medium", "Medium"),
        ("hard", "Hard"),
        ("all", "All"),  # overall aggregate
    ]

    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="topic_stats"
    )
    topic = models.ForeignKey(
        "quizzes.Topic", on_delete=models.CASCADE, related_name="user_stats"
    )
    ...
    def __str__(self):
        return (
            ...
            f" [{self.difficulty}] avg: {self.avg_score}%"
        )
...
class PersonalBest(models.Model):
    ...
    topic = models.ForeignKey(
        "quizzes.Topic", on_delete=models.CASCADE, related_name="personal_bests"
    )
